...
!!! tip
    `$$` is a **mid-path separator** — it always appears between two parts of the path. It works exactly like `$` in that the next token after `$$` is a CSS selector, but the search crosses shadow-root boundaries automatically instead of requiring each level to be spelled out.
