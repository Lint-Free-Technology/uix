...
class HaDialogPatch extends ModdedElement {
  ...
  updated(_orig, args) {
    ...
    } else if (!(coordinator?.dialogApplyAfterShow) && this._uixAfterShowListener) {
...
class HaNotificationPatch extends ModdedElement {
  ...
  async showDialog(_orig, params, ...rest) {
    ...
    this.updateComplete.then(async () => {
      ...
      if (params.id) {
        haToast = this.shadowRoot.querySelector(`ha-toast[data-notification-key='identified-${params.id}']`);
      } else {
